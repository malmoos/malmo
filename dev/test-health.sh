...
#
# What this proves:
#   1. molma-storage-verify reads a marker + canary tree and writes a
#      protocol.StorageHealth payload to /run/molma/health/storage.json
#      (here pointed at a tempdir).
#   2. The fake host-agent, wired with MOLMA_HEALTH_PATH, serves that file's
...
WORK="$(mktemp -d -t molma-health-XXXXXX)"
echo "workspace: $WORK"

SOCK="$WORK/agent.sock"
HEALTH_FILE="$WORK/storage.json"
FIXTURE_ROOT="$WORK/fixture"        # MOLMA_VERIFY_ROOT
...
mkdir -p "$FIXTURE_ROOT/etc/molma" "$FIXTURE_ROOT/srv/molma" "$FIXTURE_ROOT/var/lib/molma" \
...
reporter() {
  MOLMA_VERIFY_ROOT="$FIXTURE_ROOT" \
  MOLMA_VERIFY_OUT="$HEALTH_FILE" \
    "$ROOT/molma-storage-verify"
}
...
( cd "$ROOT" && go build -o ./molma-storage-verify ./cmd/molma-storage-verify )
( cd "$ROOT" && go build -o ./host-agent          ./cmd/host-agent )
( cd "$ROOT" && go build -o ./brain               ./cmd/brain )

# --- launch host-agent ---------------------------------------------------

step "launching fake host-agent (MOLMA_HEALTH_PATH=$HEALTH_FILE)"
...
MOLMA_AGENT_SOCK="$SOCK" \
MOLMA_HEALTH_PATH="$HEALTH_FILE" \
...
step "launching brain (MOLMA_HEALTH_POLL=500ms)"
MOLMA_LISTEN=":${BRAIN_PORT}" \
MOLMA_STATE_DIR="$STATE_DIR" \
MOLMA_CATALOG_DIR="$CATALOG_DIR" \
MOLMA_AGENT_SOCK="$SOCK" \
MOLMA_HEALTH_POLL="500ms" \
MOLMA_CADDY_ADMIN="http://127.0.0.1:1" \
MOLMA_LOG_LEVEL="info" \
...
cat >"$FIXTURE_ROOT/etc/molma/data-drive.enrolled" <<EOF
...
echo abc-123 > "$FIXTURE_ROOT/srv/molma/.canary"
echo abc-123 > "$FIXTURE_ROOT/var/lib/molma/.canary"
reporter
wait_for_issues ""
...
echo stale-uuid > "$FIXTURE_ROOT/var/lib/molma/.canary"
reporter
...
echo xyz-999 > "$FIXTURE_ROOT/srv/molma/.canary"
echo xyz-999 > "$FIXTURE_ROOT/var/lib/molma/.canary"
reporter
