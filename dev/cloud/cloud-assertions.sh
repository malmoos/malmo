...
ssh)
    ...
    # No baked host keys. Debian's postinst generates them at IMAGE BUILD time, so
    # every box provisioned from one image would share them and any holder of the
    # published image could impersonate a box to its owner's ssh client. The build
    # deletes them and malmo-sshd-keygen.service makes per-box ones on whichever
    # boot the toggle first starts the daemon — which the successful login below
    # then proves actually happened.
    ls /etc/ssh/ssh_host_*_key >/dev/null 2>&1 \
        && fail "ssh: HOST KEYS ARE BAKED INTO THE IMAGE — every box from this image would share them: $(ls /etc/ssh/ssh_host_*_key 2>/dev/null | tr '\n' ' ')"
    echo "cloud-assertions: :22 closed and ssh.service inactive at boot, nothing rendered, no baked host keys"
    ...
    # The daemon could only have answered with host keys, so they were generated on
    # this boot, by this box — the other half of the baked-keys check above.
    ls /etc/ssh/ssh_host_*_key >/dev/null 2>&1 \
        || fail "ssh: sshd answered but /etc/ssh holds no host keys — malmo-sshd-keygen.service did not run"
