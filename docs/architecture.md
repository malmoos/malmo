...
- **Control-plane stack bring-up.** host-agent launches the brain, but the brain does not yet bring up its own stack — Caddy, `malmo-ui`, and the docker-socket-proxy (M1b, #165). Until it does, the brain reaches Docker through no proxy and runs degraded (its startup `EnsureIngress`/`Reconcile`/Caddy calls fail-soft); it still binds its listener.
