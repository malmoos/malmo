...
| `catalog` | Door-1 source behind a fixed six-method facade over two sources: the baked-directory disk reader (`New`, appliance, `MALMO_CATALOG_DIR`) and the control-plane thin client (`NewRemote`, hosted — fetches `GET /catalog/sync`, integrity-digest-verifies, last-good on-disk cache, proxies+caches assets). | `lifecycle`, `api`, `cmd/brain` |
...
- **App store signing / full cutover.** A hosted box now syncs the catalog from the control plane (`GET /catalog/sync`) with integrity-digest verification and a last-good cache; the appliance still reads the baked `catalog/` directory. Not yet done: Ed25519 **signature** verification (authenticity, not just integrity), retiring the baked `catalog/` dir + `MALMO_CATALOG_DIR`, and deciding whether the appliance syncs too.
