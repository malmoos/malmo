...
| `catalog` | Door-1 source behind a fixed six-method facade. Production (every profile) uses the control-plane thin client (`NewRemote`, `MALMO_CATALOG_URL`): fetches `GET /catalog/sync`, integrity-digest-verifies, last-good on-disk cache, proxies+caches assets. The snapshot also carries the store's authored landing page (a spotlight app + category groups, store's `home.yml`) verbatim; `Home()` projects it filtered to the box's environment, mirroring the cloud's `Service.Home` (`docs/specs/APP_STORE.md` # Landing page). The disk reader (`New`) is retained only as a test constructor; no catalog is baked into the image. | `lifecycle`, `api`, `cmd/brain` |
