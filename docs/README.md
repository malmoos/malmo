...
- [`dev/catalog-import-gaps.md`](dev/catalog-import-gaps.md) — append-only ledger of platform gaps surfaced during catalog imports (an env var molma can't inject, a secret it can't generate). Import agents capture findings here; a human triages and promotes recurring/severe gaps into `specs/NEXT.md`. Capture, not the design backlog — the two are kept distinct on purpose.
- [`dev/catalog-status.md`](dev/catalog-status.md) — per-app roster of every evaluated app and its state (`Full` / `Degraded` / `Blocked` / `Rejected`), keyed by app and including the ones that are fine. The "look it up by name" companion to the (mechanism-keyed) gaps ledger; the limitation cell links into it rather than restating it.
