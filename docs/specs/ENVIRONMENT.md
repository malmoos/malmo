...
The box-side machinery for the always-on wildcard scheme is realized (C3b, #207). What real Let's Encrypt issuance needs — public DNS, the cloud's acme-dns face — is exercised jointly in the cloud on-ramp (cloud #6 / CL6), not the inner loop. The mechanism:
