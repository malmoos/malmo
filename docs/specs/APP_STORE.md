...
The box derives nothing else from the block: it does not compute rank and does not re-sort groups. It also does not look up `categories.yml`'s authored `label:` for the group headings — that field isn't on the wire — but it does not render the bare id either: both the box and the marketing store apply the same display-only stopgap (hyphens to spaces, e.g. `developer-tools` → "developer tools") so the two surfaces don't visibly disagree with each other while the real fix (putting `categories.yml`'s label on the wire) is still a separate cloud+store change. If a synced snapshot carries no home block (an older cloud, or a store publish with an empty `home.yml`), the box's landing has no spotlight and no groups; the view falls back to the flat curated Featured row, and if that's empty too, to a plain "pick a category or search" prompt — the landing is never blank, but an empty home block is not the same as "nothing curated."
