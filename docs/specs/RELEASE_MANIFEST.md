...
- **Pointing a test box elsewhere.** `MALMO_RELEASE_BASE_URL` overrides the `releases.malmo.network` base so a box under test can read a manifest from a local file server. It is safe to expose because **the base URL is not the trust boundary — the signature is**: a manifest served from anywhere still has to verify against the baked keys, so an override cannot be used to feed a box an unsigned or wrongly-signed manifest. Empty means the real releases host.
