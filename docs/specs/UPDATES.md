...
- **As built (#404), both the target and the window are systemd credentials on `host-agent.service` — which means neither is actually settable on a provisioned box.** The claim this was built on, that credentials are "the same path `malmo.seed` already arrives on", was wrong: `malmo.seed` is a credential only in the QEMU lane, and a real box gets its per-box facts as metadata user-data, which does not feed systemd's credential store (`ENVIRONMENT.md` # Provisioning & first-boot — the write-once channel bullet). The mechanism works in the test lane and is inert everywhere else. #407 moves the target URL into the seed, where a fact fixed at VM-create belongs, and #408 moves the window into the control plane's answer, because the window has to be changeable while the box runs and the seed can never be rewritten. Everything else below stands. `malmo.update_target_url` sets the endpoint the box reads; `malmo.update_window` sets the window it applies in. This is what makes per-box pinning possible at all: a hosted box has no SSH (`ENVIRONMENT.md` # Access & files), so before this there was no way to point an already-provisioned box at `?channel=candidate`. The precedence is **credential, then environment variable, then the built-in default**: the credential is the per-box fact from the provisioner, and `MALMO_UPDATE_TARGET_URL` / `MALMO_UPDATE_WINDOW` stay below it as the local hand-edit an operator drop-in or the CI boot proof uses. A box given neither credential behaves exactly as it did before. Which source won is logged once at startup, and a box on a non-default target logs it at **warn** level, because "this box is not following the fleet" is the first thing to know when one box behaves unlike the rest.
- **An unusable target credential is refused, not resolved away (#404).** If `malmo.update_target_url` is present but is not an absolute `http` or `https` URL with a host, host-agent logs an error and **does not start the update loop at all**. It does not fall back to the fleet endpoint: a box that was deliberately pinned to a candidate must never quietly join stable because of a typo. The box keeps serving whatever it is already running. An unusable **window** is not treated the same way and falls back to 03:00-04:00 with a warning, because a wrong hour can only apply an update at the wrong time, while a wrong target sends the box to the wrong version.
