...
- **What counts as an expected repository is configuration, not a constant.** The default pair is `ghcr.io/malmoos/brain` and `ghcr.io/malmoos/ui` (`BUILD.md` # 6 — public, pulled by digest), and each is overridable on its own: `MALMO_UPDATE_BRAIN_REPO` and `MALMO_UPDATE_UI_REPO`. They exist because the CI boot proof serves both images from a registry **inside the guest**, and because a box under test may be pointed at a different registry. They are not a trust boundary on their own — the digest is what pins the bytes — but they are the check that stops an otherwise well-formed answer from sending a box to somebody else's image. Overriding them on a production box is the same class of act as pointing it at a non-default target: deliberate, and worth knowing about before it is done.
