...
**App state** is the app's own working data — indexes, caches, databases, configs. Lives at `/var/lib/malmo/state/instances/<id>/data/`. Opaque to the user. Deleted on uninstall (or archived if the user picks "keep data").
...
- **User content vs. app state are separate stores.** User content (`/home/<user>/Photos/`, etc.) accessed by manifest-declared bind mounts of use-case folders; app state in `/var/lib/malmo/state/instances/<id>/data/`. Apps reach user content by reference, never by copy.
...
- **Image references use version tags; the store catalog resolves digests.** Authors write `image: foo/bar:1.2.3`; malmo's CI pins the bytes via a `sha256:` digest in the published catalog (`APP_STORE.md`). Door-2 custom apps fall back to TOFU digest pinning in the brain.
