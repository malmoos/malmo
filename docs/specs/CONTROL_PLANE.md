...
- **`EXEC` stays denied; managed-DB-in-production is gated on a provisioning re-architecture.** Managed databases (`internal/lifecycle/services.go`) currently provision per-app roles/databases by `docker exec`'ing the engine's client (`psql`/`mysql`/`valkey-cli`), which the proxy denies. Rather than widen `EXEC=1` (which would defeat this mitigation), the containerized brain ships with `EXEC` denied and managed-DB-in-production is deferred until provisioning is re-architected off `docker exec` (engine-over-TCP or a one-shot provisioning container). The **dev** brain is unaffected — it runs natively with the raw socket and never sets `DOCKER_HOST` — so managed DB keeps working in the inner loop; only the containerized/production brain is gated (`DECISIONS.md` 2026-06-14).
