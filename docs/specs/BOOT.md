...
- **`host-agent.service`** orders `After=malmo-storage-ready.target docker.service`, `Wants=` both. Reads `/run/malmo/health/storage.json` after startup and forwards findings to the brain via the existing protocol. It carries `RuntimeDirectoryPreserve=yes` so stopping it does **not** delete `/run/malmo` — that directory holds both the agent socket the brain bind-mounts and this reporter's `storage.json`, and the reporter is a `Type=oneshot` that runs once at boot, so a deleted `storage.json` is never rewritten (#447, `CONTROL_PLANE.md` # Locked: host-agent runs under systemd).
