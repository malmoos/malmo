...
For malmo's audience (non-technical home users), a box that refuses to boot is a worse failure than a box that boots into a degraded state with a banner explaining what's wrong. A user cannot debug a dark box without help; they *can* read a banner and click a button.
...
The systemd-level recovery target (`malmo-recovery.target`) exists for the small set of failures where the brain genuinely cannot run — there's no UI to surface anything from, so a static rescue page is the only option. Everything else flows through degraded mode.
     ...
     └─ LUKS unlock (root drive) via TPM2 PCR 7
        └─ root mounted
           └─ systemd userspace starts
              │
              ├─ malmo-storage-ready.target              ← best-effort assembly target
              │     Wants:
              │       • cryptsetup@data.service          (if marker present)
              │       • srv-malmo.mount                  (if data drive present)
              │       • srv-malmo-mergerfs.mount         (mergerfs union)
              │       • home-<user>.mount, …            (bind mounts, per user)
              │       • var-lib-malmo.mount              (bind mount)
              │     After:
              │       • malmo-storage-verify.service     (reporter — writes findings to /run/malmo/health/)
              │
              ├─ nftables.service                        (Before=docker.service)
              ├─ network-online.target                   (scoped to primary iface)
              │
              ├─ docker.service                          (After=malmo-storage-ready.target Wants=malmo-storage-ready.target)
              │     └─ host-agent.service               (Type=notify, After=docker; reads /run/malmo/health/)
              │         └─ malmo-brain container       (launched by host-agent; reads health findings)
              │             └─ malmo-caddy container   (launched by brain)
              │
              ├─ Tier-2 host services                    (After=malmo-storage-ready.target Wants=…)
              │     • avahi-daemon.service
              │     • smbd.service                       (drop-in: refuse writes if storage findings flag it)
              │     • tailscaled.service
              │     • sshd.service                       (off by default; AUTH.md)
              │
              └─ malmo-prepare-wizard.service            (oneshot, runs if not bootstrapped)
```
...
`malmo-storage-ready.target` is the milestone for "storage assembly has been attempted." It is *not* a strict precondition for the brain. Its job is to:

1. Trigger the LUKS unlock of the data drive (only if the box has one enrolled — see `STORAGE.md` # Data drive enrollment marker).
2. Trigger `/srv/malmo` mount (or the OS-drive fallback at Level 0).
3. Trigger the mergerfs union mount over the data branches (when a data drive is present).
4. Trigger all bind mounts for `/home/<user>` and `/var/lib/malmo`.
5. Run `malmo-storage-verify.service` (`Type=oneshot`) — the **reporter**.

The reporter does not gate anything. It writes its findings to `/run/malmo/health/storage.json`:
...
**Why this is the right shape:** the historical alternative was a strict gate that refused to start malmo userspace if storage looked wrong. That defended against the rare silent-wrong-tree corruption class at the cost of bricking the box for the much more common "drive came unplugged" or "user replaced the data drive" cases. The new shape lets the brain refuse the dangerous operations (writes to user data, app installs) while still coming up to tell the user what's wrong and let them fix it.

### What downstream services do

- **`docker.service`** orders `After=malmo-storage-ready.target Wants=malmo-storage-ready.target`. Docker comes up even if storage assembly partially failed — host-agent and brain handle the consequences. Docker's own data root issue (see below) is mitigated by Docker checking the data root path is on the expected filesystem before initializing.
...
`malmo-recovery.target` exists for the small set of failures where the brain genuinely cannot run. There's no dashboard to surface health issues in, so a separate static rescue page is the only option.
...
| host-agent crashloop (5 starts in 60s) past `StartLimitBurst` | host-agent binary itself broken (failed update, corrupt image) | `malmo-recovery.target` activates; static page on port 80 offers "Roll back to previous host-agent version" |
...
**`OnFailure=malmo-recovery.target` is set only on `host-agent.service`.** Not on `malmo-storage-verify.service` (it's a reporter, doesn't fail). Not on individual mount units (they can fail without bricking the box).

### What recovery target serves

`malmo-recovery.service` is a tiny static-page server bound to port 80, independent of `/var/lib/malmo` and the data drive (binary lives on the OS drive, serves from `/usr/lib/malmo/recovery/`). It publishes its own mDNS name (`malmo-recovery.local`) so the user can find it from another device on the LAN — avahi is not gated behind the storage target for this reason.
...
Docker writes its own iptables/nftables rules at startup for container NAT and forwarding. malmo's rules live in a dedicated `inet malmo` table (separate family/table from Docker's). Running before Docker ensures our table is in place when Docker brings up container networking; the two never touch each other's rules.
...
Upstream Debian ships `smbd.service`, `avahi-daemon.service`, etc. malmo extends them via `/etc/systemd/system/<unit>.service.d/malmo.conf` drop-ins that add `After=malmo-storage-ready.target Wants=malmo-storage-ready.target` (and any other malmo-specific ordering). Apt updates to the base unit don't fight us. **Drop-ins use `Wants=`, not `Requires=`, for the same reason as the rest of the chain.**
...
> **As built, the marker is not a file.** The brain records completion as `first_run_complete` in its `box_meta` table (`internal/store`), surfaced on `GET /api/v1/auth/state` and set by `POST /api/v1/system/first-run-complete`. The `/var/lib/malmo-state/.bootstrapped` file this section described was never created, and neither was that path root — the brain's state lives at `/var/lib/malmo/state/` (`STORAGE.md` # mount layout). `malmo-prepare-wizard.service` itself does not exist yet either (`dist/systemd/` ships the storage + recovery units and `host-agent.service`), so a unit that lands later must read the brain's state, not a file beside it.

### Hang protection on the storage chain

`malmo-storage-verify.service` has `TimeoutStartSec=60s`. A hang in the verifier (e.g., a wedged disk driver) trips the timeout, the reporter writes a `storage-verify-timeout` finding, and the brain raises the corresponding health issue. The boot does not stall.
...
- **Recovery dashboard UX (the page served by `malmo-recovery.target`)** → `RECOVERY.md`, deferred (`NEXT.md`).
