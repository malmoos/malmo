...
- **B — host-agent periodic reporters.** host-agent samples on a timer and the brain polls a health endpoint, reconciling findings the same way boot findings are reconciled. *(Built: `GET /v1/health/system` @ 60s — storage category from the boot reporter, the `service-down` services category via `systemctl is-active`, and the `clock-not-synced` time category via `chronyc tracking` (re-queried ≤ every 5 min); `ApplyFindings(category, …)` reconciles each domain independently.)* Everything host-physical and recurring lives here.
