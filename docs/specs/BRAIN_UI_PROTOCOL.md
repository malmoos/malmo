...
`icon_url` and the `screenshot_urls` entries point at `GET /api/v1/catalog/:id/icon` and `/screenshots/:n` — they serve **raw image bytes**, not JSON, so the store loads them directly in `<img>` tags (and they stay out of the OpenAPI surface). `icon_url` is present only when the manifest declares an icon, so the store renders a glyph fallback without ever requesting a 404 — the manifest's optional `icon_glyph` (a Lucide name, returned as a plain JSON string on the `Entry`) picks that fallback glyph; absent ⇒ a generic glyph. **As built (#420) these are proxied, not read off disk.** No catalog is baked into the box and the box keeps none (`APP_STORE.md`), so the brain resolves an asset by fetching it from the control plane on first request and caching the bytes under `MALMO_CATALOG_CACHE_DIR` — a cache with a 24h expiry, keyed per app, never a catalog directory. What the endpoint serves is therefore the last good copy of a remote file, and a box that has never synced serves none. An unknown app, an app that declares no icon, an out-of-range index, or a non-numeric `:n` → 404.
