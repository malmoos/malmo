...
**What "client IP" means.** A per-IP bucket is only a throttle if the key is something the caller cannot choose. The brain reads `X-Forwarded-For` **only** when the request's immediate peer is a configured trusted proxy, and then takes the **last untrusted hop** in the chain rather than the first; from any other peer the header is ignored entirely and the peer address is the key. The trusted set defaults to the private ranges — the brain container publishes no port, so every peer it can have is on the box's own Docker network — and is narrowed or widened with `MALMO_TRUSTED_PROXIES`. Caddy, as the box's edge, trusts **no** client: it declares an empty trusted-proxy set and so replaces an inbound `X-Forwarded-For` with the address it actually saw. The same rule and the same derived address govern the login throttle (`AUTH.md` # Rate limiting) and what the audit log records as a request's origin.
