...
> How molma maps dashboard roles to Linux accounts and groups, what privileges each role has on the host, and the rescue path when the dashboard can't help. Companion to `AUTH.md` (credentials, sessions), `STORAGE.md` (group-based file permissions), `FIRST_RUN.md` (account creation), `BRAIN_HOST_PROTOCOL.md` (the `molma` group as IPC channel).

## Principle: UI is the path, SSH is rescue

**Every privileged operation on a molma box has a UI path.** Adding a user, opening an SMB share, changing the network, installing a Tier-2 service, restarting an app — all of it lives in the dashboard. The dashboard talks to host-agent for anything that requires root; the user never types `sudo` in normal use.
...
- **UID range:** ≥ 3000. Below 3000 is reserved for the system and molma internals.
...
|----------------|----------------------------------------|-------------|------------------|
| **Member**     | own group, `molma-shared`              | No          | Unprivileged user. Reads own files, household shared, that's it. |
| **Admin**      | own group, `molma-shared`, `sudo`      | Yes         | Can become root via `sudo`. Recovery path when the brain is broken. |
...
If a tinkerer wants a non-molma user account with sudo for shell-only work, they can create one manually (`useradd`, `usermod -aG sudo`). That account won't appear in the dashboard, won't have a brain identity, and won't be a "molma user" — it's outside the system. We don't document this as a supported pattern; we don't break it either.

## Group reference

All groups present on a molma box, why they exist, and who's in them.

| Group           | GID range | Purpose                                                                                   | Members |
|-----------------|-----------|-------------------------------------------------------------------------------------------|---------|
| `<user>`        | ≥ 3000    | A user's own primary group. Created with the account.                                     | The user themselves. |
| `molma-shared`  | fixed     | Read/write access to `/srv/molma/shared/` (household-shared content). Setgid on the dir.  | Every molma user (member or admin). Plus shared service UIDs (e.g., a household Jellyfin). |
| `sudo`          | system    | Standard Debian — members of this group can `sudo`.                                       | Admin users only. |
| `molma`         | fixed     | Access to the host-agent UNIX socket at `/var/run/molma/agent.sock`. Kernel-enforced IPC boundary. | The brain's container runtime UID. **Nothing else.** |

**`molma` vs. `molma-shared` are unrelated, despite the similar name.** `molma` is the IPC channel between brain and host-agent (one member, enforced by a CI test — `AUTH.md` # Authentication between brain and host-agent). `molma-shared` is the household-content permission group (many members, household-wide). Don't conflate them.
...
3. Common rescue operations:
   - `systemctl status molma-host-agent` — is host-agent running?
   - `journalctl -u molma-host-agent -n 200` — recent errors.
   - `docker ps -a` — is the brain container up?
   - `docker logs molma-brain --tail 200` — brain logs.
   - `systemctl restart molma-host-agent` — restart the daemon.
   - `docker restart molma-brain` — restart the brain.
4. If the brain's SQLite is corrupt: stop the brain, replace `/var/lib/molma/brain/state.db` from the most recent snapshot (snapshot/restore story is its own doc — not yet written), restart.
...
- **Group management UI for `molma-shared`.** Currently every user is in `molma-shared` at creation. Kicking a user off household-shared content (without deleting the account) has no UI surface yet — see `NEXT.md`.
...
- **The `molma` group has exactly one member** (the brain's runtime UID). CI-enforced — see `AUTH.md` # Authentication between brain and host-agent.
