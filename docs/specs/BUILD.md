...
- The file holds plain `NAME=name:tag@sha256:...` lines, so it is both `include`-able by the `Makefile` and `source`-able by shell. The tag half stays readable as a label; the digest decides the bytes. Each digest is the multi-arch **index** digest, so the pin does not assume an architecture.
