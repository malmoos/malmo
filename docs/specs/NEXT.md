...
The hosted cloud image enforces **no malmo-managed host firewall** — L3/L4 filtering is the cloud provider's security groups / VPC firewall, recorded as an explicit operator requirement (`DECISIONS.md` 2026-06-19, #203). That is the right default for a provider-fronted VM, but it leaves the Docker-publishes-to-`0.0.0.0` behavior acceptable *only* because the provider firewall is assumed present. If a hosted target ever lacks a security-group layer (a bare provider, an on-prem "hosted-style" deploy), a minimal in-guest `nftables` default-deny (admit 443 + the ACME/redirect 80, drop the rest) would be the in-VM backstop. The `nftables` package itself is already in the image (docker-ce hard-Depends on it as its firewall backend, `DECISIONS.md` 2026-06-23) — only the malmo-owned ruleset is absent, so the backstop is a ruleset + a host-agent seam, not a new package. Pin the ruleset shape and the host-agent seam that would own it before that scenario is real.
...
### Managed MongoDB (FerretDB engine) — evaluated, deferred (do not relitigate without the gates below)

A managed `type: mongodb` service (the redis→valkey license substitution applied to Mongo, with [FerretDB](https://www.ferretdb.com/) v2 underneath since MongoDB Community is SSPL) was evaluated end-to-end in a Phase 0 compatibility spike (#253, `docs/progress/mongodb-compat-spike.md`, 2026-06-25). **Verdict: NO-GO for v1, on two independent measured findings.** (1) **Compatibility ceiling:** FerretDB v2 / DocumentDB has no change streams, no oplog, no MongoDB replica set, and no multi-document transactions — so the marquee demand (Rocket.Chat) is hard-blocked, Habitica/Appsmith are blocked, Wekan only runs degraded (Meteor poll-and-diff), and the genuinely-Mongo-only apps that work cleanly come to ~1 (LibreChat) + 1 degraded (Wekan), short of the 3+ bar. (2) **Isolation:** FerretDB v2 enforces authentication but **not** MongoDB per-database authorization — a per-app-scoped user has full read/write to every other app's database on a shared instance (the proxy has only a `--auth` flag, no RBAC). That violates the `SERVICE_PROVISIONING.md` # Per-app isolation contract every other Tier-1 service upholds, and the only workaround (one instance per app) defeats the shared-instance model.

**Revisit only when *both* upstream gates clear:** (a) DocumentDB ships change streams ([microsoft/documentdb#50](https://github.com/microsoft/documentdb/issues/50), upstream-targeted Q3 2026) — the demand driver; **and** (b) FerretDB/DocumentDB gains enforced per-database authorization — without it the type cannot ship as a shared Tier-1 service regardless of app compat. The spike records the provisioning-client image (a self-built Apache-2.0 `mongodb-mongosh` image), the two-container compose topology (proxy = readiness handle, store = volume), the corrected DSN shape (`?authSource=admin&directConnection=true&retryWrites=false`), and the per-app verdict table, so a future revisit starts from the gates, not from scratch.

**Context:** `SERVICE_PROVISIONING.md` # Post-v1 candidates (MongoDB), `docs/progress/mongodb-compat-spike.md`, the redis→valkey precedent (`DECISIONS.md` 2026-06-13).
