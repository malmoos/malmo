...
- **Image download size in manifest and install dialog.** `estimated_size` (`APP_MANIFEST.md` # Storage) covers *app state on disk*, not the container image download. A 5 GB image is a meaningful install-time signal (disk + bandwidth) that users should see before confirming. Open: add a `storage.image_size` field (or equivalent) that catalog CI auto-populates from the registry manifest list at publish time, and surface it in the install dialog alongside `estimated_size`. `APP_MANIFEST.md`, `APP_STORE.md`.
