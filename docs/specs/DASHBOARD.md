...
- **Failed** — an instance whose install or start transaction ended in `failed` (`APP_LIFECYCLE.md` # install transaction, # stop, start, uninstall). The tile takes a **light amber/warning tint** — deliberately distinct from the gray *stopped* tile — and keeps the corner **alert mark**: failed is trouble, not an intended state. A viewer who may control the app (admin for any app; the owner for their own personal app) can **click the tile to retry**, with a hover caption reading "Failed — click to retry"; while the retry job runs the tile shows a "Retrying…" caption until `main_service` is healthy, then the tile returns to nominal (or back to the amber failed state if it still won't come up). The retry runs the **same Start transaction** as a stopped-app start (`APP_LIFECYCLE.md` # stop, start, uninstall), including the `<slug>.local` mDNS re-publish, so on success the app is `running` with its name and route restored. A controller also gets a **"View details" link** to the app page (`/settings/apps/<id>`), where the failure reason / logs live — so a persistent failure is diagnosed rather than blindly re-clicked. A viewer who can't control the app sees the amber alert tile without the retry affordance or the link.
...
Clicking a **running** tile **opens the app in a new browser tab** at its own host (`<slug>.local` or `<slug>--<user>.local` depending on whether disambiguation was needed, or the `.malmo.network` host when the remote toggle is on). The app runs on its own origin — that's the whole point of subdomain routing (`SPEC.md`: browser same-origin isolation). The dashboard is the launcher, not a frame/proxy around apps. (A grayed *stopped* tile is the one exception — clicking it starts the service rather than opening it; see # Tile above.)
