...
### smtp-relay — ghost (2026-06-09)

- **Severity:** degraded
- **Trigger:** `mail__transport` / `mail__options__*` — Ghost's transactional email (staff invites, member magic-link sign-in, password resets) needs an outgoing SMTP transport configured via env; unconfigured, Ghost falls back to nodemailer's Direct transport, which is dead on arrival from a home connection (residential IPs are blocklisted and ISPs commonly block outbound :25). Newsletter delivery is separate — Ghost's bulk mail is Mailgun-only, configured inside its own admin UI.
- **What breaks:** every email Ghost sends — inviting a second staff writer, member sign-up/sign-in (Ghost members are passwordless magic-link only; there is no password fallback), password resets, newsletter sends. The blog, editor, and admin all work fully without mail; a single-author site without members never notices.
- **Why molma can't satisfy it (v1):** molma has no outgoing-mail mechanism — no SMTP relay to point apps at, no `MOLMA_SMTP_*` injection, and no per-app env-override UI through which a user could supply their own provider's credentials post-install. (The Mailgun newsletter path needs nothing from molma — it's configured in Ghost's admin settings.) Recurs for effectively every multi-user app: password-reset and invite flows are near-universal.
- **Status:** open
