...
- **Status:** planned (#108) — promoted to a managed **MySQL + MariaDB** Tier-1 service type (both engines, per the product call to support both). Recurs for Kimai (#89). Reported on #85, assigned to onel for the platform/curation call.
...
- **Status:** planned (#108) — no `catalog/kimai/` files written. Blocked on the same managed MySQL + MariaDB mechanism as Ghost; if/when that ships, both apps are re-importable. Reported on #89, assigned to onel.
