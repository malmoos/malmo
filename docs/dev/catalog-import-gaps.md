...
- **Status:** mechanism implemented (#122 — BYO outgoing mail, `SERVICE_PROVISIONING.md` # BYO outgoing mail); gitea's own manifest/compose not yet updated. The revisit: declare `mail: {optional: true}` and map `GITEA__mailer__ENABLED` + `GITEA__mailer__SMTP_ADDR`, `SMTP_PORT`, `FROM`, `USER`, `PASSWD` from the injected `MALMO_MAIL_*` vars (Gitea has no single-DSN var, so it consumes the discrete fields; `ENABLED` needs a guard for the unbound case, e.g. defaulting off when `MALMO_MAIL_HOST` is absent).
