...
- **Status:** open — recurs for Kimai (#89, MySQL/MariaDB). To unblock: add a managed **MySQL** service type (extend managed services beyond Postgres/Redis). Reported on #85, assigned to onel for the platform/curation call.
...
- **Status:** open — no `catalog/kimai/` files written. Blocked on the same managed-MySQL mechanism as Ghost; if/when that ships, both apps are re-importable. Reported on #89, assigned to onel.
