...
- **Why malmo can't satisfy it (v1):** managed redis is explicitly staged-not-provisioned (`SERVICE_PROVISIONING.md`; `NEXT.md` carries the worker design). Workaround that holds today: ship redis as an app-compose sidecar — verified clean under the Tier-3 sandbox as an elected non-root uid (`redis:7.2`, `cap_drop: ALL`, 2100-owned `/data` bind). Postiz itself stayed blocked for unrelated reasons (see `nonroot-data-ownership — postiz`), but the sidecar pattern is sound for other redis-needing apps.
- **Status:** open
