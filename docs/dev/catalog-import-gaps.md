...
- **What breaks:** the patch can't run under the sandbox, so the max request body size stays at the Next.js framework default (~1mb). Note attachments / images larger than ~1mb fail to upload; text notes, checklists, and Markdown — the core of the app — are unaffected.
...
- **Status:** open — shipped degraded (`catalog/jotty`). To unblock the upload limit specifically: upstream making the limit a plain runtime setting instead of a `node_modules` patch. The broader pattern (self-chowning / privilege-dropping / self-patching entrypoints under `cap_drop: ALL`) recurs and is captured here for triage.
