...
### content-provisioning — kiwix (2026-06-07)

- **Severity:** blocks-start
- **Trigger:** `kiwix-serve` serves ZIM archives and requires either `ZIMPATH ...` or `--library LIBRARYPATH` — there is no auto-scan and no empty-catalog mode. The image's `start.sh` runs `kiwix-serve --port=8080 $@`, so with no ZIMs and no library it exits.
- **What breaks:** a fresh install has no ZIM content, so the container exits 1 on first boot — and there is no path for a non-technical molma user to fix it. Verified by running `ghcr.io/kiwix/kiwix-serve` (kiwix-tools 3.8.2) under the sandbox (`--cap-drop ALL --security-opt no-new-privileges:true`, root-owned `/data`): `--library /data/library.xml` → "Failed to load the XML library file"; a `/data/*.zim` glob → "Unable to add the ZIM file". ZIM files are multi-GB downloads from the Kiwix library, and registering them needs `kiwix-manage` (CLI-only); molma has no in-app download or library-management UI and SSH is rescue-only (the north star is non-technical users).
- **Why molma can't satisfy it (v1):** Kiwix is a content-pack app — inert until the user provisions large external content that molma offers no mechanism to acquire, place, or register, and it won't boot without that content. Compounding it, the image runs as a non-root user (uid 1001), so it can't write molma's root-owned instance data dir (verified: `touch /data/library.xml` → `Permission denied`) — the same `nonroot-data-ownership` gap seen for Poznote — which kills the image's `DOWNLOAD` env path and any in-place library creation.
- **Status:** open — no `catalog/kiwix/` files written. This is a curation/product decision: whether and how molma supports content-pack apps (a content-acquisition + library-management UX, and/or a writable content folder owned by the app's runtime user). Reported on #86, assigned to onel.
