...
1. GATHER. Given a repo URL, do NOT clone — read just the files you need via `gh`:
   ...
   - **Screenshots**: Parse the README for embedded images (`![...](...)` markdown). Only consider repo-relative paths — if all images link to external URLs (CDN, GitHub issue attachments, Notion, etc.), skip screenshots entirely and move on. For repo-relative paths, download each via `gh api repos/<owner>/<repo>/contents/<path> -H "Accept: application/vnd.github.raw" --output catalog/<id>/screenshots/NN.<ext>`, numbered `01`, `02`, etc., preserving the source extension (png/jpg/gif/webp).
