...
13. RECORD THE VERDICT. If the app shipped clean, you're done. If a real feature degrades, the user-visible limitation belongs in the manifest's `description.long` (so a user sees it before installing) and the step-12 ledger entry (so the mechanism is captured) — the app still ships listed. If you wrote a manifest but discovered it can't run at all (crash-loops / never serves), set `listed: false` in its manifest so the store withdraws it — a `Blocked` app (re-shippable once a named gap closes) or a `Rejected` one (never) — see `APP_STORE.md` # Listed apps. A fully bailed app (no files written, per step 11) ships nothing.
...
- **Confirm the `listed` flag.** Any app you marked `Blocked` or `Rejected` must actually carry `listed: false` in its manifest, so the store's behavior matches the curation verdict.
