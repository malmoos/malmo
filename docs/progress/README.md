...
1. **Notification retention / pruning.** The **mute settings-toggle UI** landed in [notification-mute-settings-ui.md](notification-mute-settings-ui.md) (Settings → Notifications per-category switches over the mute API from [notification-category-mute.md](notification-category-mute.md)), closing the UI half of the notification line. Remaining: **retention/pruning** for the `notifications` + `notification_reads` tables (`NEXT.md` # Observability) — cap by count and/or age, resolved-rows-first, and pick where the prune runs. Brain-side, no UI. Follow-up from [notification-mute-settings-ui.md](notification-mute-settings-ui.md).
