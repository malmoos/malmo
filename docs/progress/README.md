...
1. **Notification follow-ups: member transparency variant + "all clear" + per-category mute.** [notification-web-ui.md](notification-web-ui.md) landed the dashboard bell (UI); [notification-read-surface.md](notification-read-surface.md) the read surface; [health-notifications.md](health-notifications.md) the write seam. Remaining: emit the info-only member **transparency variant** for box-wide criticals that block a member-visible function, and the **"all clear"** resolved follow-up on clear (both brain, `NOTIFICATIONS.md`); then per-user **per-category mute** (`NOTIFICATIONS.md` # Configuration); then **retention/pruning** for `notifications` + `notification_reads` (`NEXT.md`). The bell already renders the transparency / all-clear copy once a producer emits it. Follow-up from [notification-web-ui.md](notification-web-ui.md).
