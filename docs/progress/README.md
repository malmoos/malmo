...
1. **Notification follow-ups: per-category mute + retention.** [0028](0028-notification-clears-transparency.md) landed the member **transparency variant** and the **"all clear"** on resolve (brain emit path); [0027](0027-notification-web-ui.md) the dashboard bell (UI); [0026](0026-notification-read-surface.md) the read surface; [0025](0025-health-notifications.md) the write seam. Remaining: per-user **per-category mute** (`NOTIFICATIONS.md` # Configuration) — a prefs table + a filter in the list/count queries + a settings toggle; then **retention/pruning** for `notifications` + `notification_reads` (`NEXT.md` # Observability). Follow-up from [0028](0028-notification-clears-transparency.md).
