...
- **`malmo manifest resolve <manifest.yml>`** — the second `manifest` subcommand. It reads the sibling compose, resolves each distinct image's `{digest, download_bytes, disk_bytes}`, rewrites the manifest's `images` block in place, and prints the per-image sizes + the derived footprint. **Fails loudly** — a single unsizable image aborts the whole run with no write, so the catalog never carries a bogus zero (Done-when #3).
- **`dockerSizer`** drives the local Docker daemon (the maintainer's call over adding `go-containerregistry`; consistent with `internal/lifecycle/pinning.go`, which already shells to docker, and no new dependency): `docker manifest inspect` for the compressed download size (the amd64 sub-manifest's layer sizes — malmo is x86-only), a `--platform linux/amd64` pull + `docker image inspect {{.Size}}` for the uncompressed disk size, and the pulled image's RepoDigest for the pinned (index) digest. It is behind an `imageSizer` interface so the parse/footprint/write-back logic is unit-tested with a fake.
...
- **Catalog-repo CI** that runs `malmo manifest lint` + `malmo manifest resolve` over the tree and regenerates `catalog.json` (`APP_STORE.md` # CI on the repo); a daemon-free sizer fits there.
