...
- **A trailing comment on the `image:` line poisoned the returned ref, and review caught it.** The first version returned everything after `image:`, so `image: malmo-ui:dev # baked at build` yielded the comment as part of the ref — and that ref is what a caller records as the previous generation, which is what a revert pins. The revert would have tried to run a comment as an image, failing at the one moment the box most needs it to work. The committed compose carries no such comment, which is exactly why this would have gone unnoticed until someone annotated the line. Fixed with `stripInlineComment`, and the test is mutation-checked.
