...
Its second pass showed the first redaction was **half a fix**, and the measurements are worth keeping. `http.Client.Do` returns a `*url.Error` that strips the password but keeps the whole query; `url.Parse` returns one that keeps everything, password included — and both were wrapped with `%w` straight into `detail`. So a box pointed at `https://host/target?token=secret` still leaked, and a malformed seeded URL leaked its password in cleartext. `RedactURL` now drops the query and fragment as well as the userinfo (replacing the query rather than deleting it, so a diagnostic still says one was there), and `CauseOf` unwraps `*url.Error` to the failure underneath so the raw URL cannot ride in alongside the redacted one. Both hiding places are now covered by tests on the fetch path and on the seed path.
...
The `update` boot went red twice on the new assertion, and the second failure was worth the trip: **it is a real box bug, filed as #447.**

`host-agent.service` declares `RuntimeDirectory=malmo`, and systemd removes that directory when the unit stops, then makes a fresh one on start — a new inode. The brain reaches the socket through a **directory** bind mount (`brainlaunch.runSpec` mounts `filepath.Dir(cfg.SocketPath)`), resolved by Docker at container-create time. So after `systemctl restart host-agent`, the brain's mount points at the deleted directory and it never sees the new `agent.sock`. Every host-backed brain call fails until the brain **container** is recreated.

Measured on real Docker with no malmo involved: bind a directory into a container, delete and recreate that directory on the host, and the container's view stays empty forever. In the lane the same shape showed as `GET /api/v1/me` answering 200, all six containers `Up`, and the one endpoint in that path that makes a host call answering 502 for as long as it was polled.

It has been invisible because nothing restarts host-agent on a healthy box, so no boot proof made a host-backed call in that window, and the endpoints that do call host-agent mostly degrade quietly by design — `GET /api/v1/system/version` omits `host_agent_version` and still answers 200, which is exactly the posture that hides it. It is not hypothetical: the unit carries `Restart=on-failure`, and stream A updates host-agent through `unattended-upgrades`, which restarts it.

**Not fixed here.** It is outside this issue and gets its own change (#447). What changed instead is where the assertion sits: the brain-side read is asserted **after** the apply, which recreates the brain container, so it tests this endpoint rather than #447. The refusal is still proven, by the journal line the lane already checked, and `refused` reaching the wire is covered by unit tests on both sides. The first failure was mine and is fixed too: the assertion now waits for `/api/v1/me` and says "the box is not serving" when that is the truth, so one 502 stops standing for two different failures.

## Known gaps & deviations

- **The cloud lane has not gone green yet.** `CI / Go` and `CI / web-ui` are green. `CI / Cloud image` is the issue's real acceptance gate for the hosted half and has not passed on this branch; until it does, the read assertions are written and not demonstrated.
- **The brain-side read of a `refused` answer is not proven on a booted box**, because the only window where a real box is in that state is the one #447 breaks. It is covered by unit tests on both sides of the socket, and the refusal itself is proven in the lane through the journal. It should be added to the lane when #447 lands.
...
- **The `disabled` report still omits `from`, and that is deliberate.** The review asked for the seed provenance there. Only one of the three ways that path is reached actually knows it: a bad seeded URL is `seed`, but an unreadable seed and an unparseable one do not tell us whether a target was set at all, so a blanket `from: "seed"` would be a guess presented as a fact. `detail` already names the setting at fault. `auto_apply` stays `false` on that path for the same kind of reason: the box will apply nothing, whatever profile it is, and reporting `true` would claim it patches itself.
