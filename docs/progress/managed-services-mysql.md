...
- **`serviceName` folds version dots to dashes** (`mysql-8-0`): compose project names reject dots, and the container name / network / DNS alias / project all share the stem — so `malmo-svc-mysql-8-0` and `mysql-8-0.malmo.internal`. Postgres names are unchanged (no dots).
...
- **`writeServiceDir`/`mysqlServiceCompose`**: same shape as the Postgres compose — external `--internal` network, versioned DNS alias, fixed `container_name`, `malmo.service` label, data at `/var/lib/mysql`, root password via `.env`.
