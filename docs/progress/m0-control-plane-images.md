...
- **Boot-verify the medium-lane image** (`sudo make test-medium-qemu` on a host with mkosi/swtpm/QEMU): confirm `docker.service` is active and `docker images` lists `malmo-brain`, `malmo-ui`, `caddy`, and `docker-socket-proxy` after first boot. A **temporary** assertion block was added to `dev/test-qemu/medium-assertions.sh` (clearly marked `TEMPORARY (M0, #163)`) that checks exactly this — `docker.service` active + the four bundled images present + the load marker — so `make test-medium-qemu` verifies the M0 "Done when" directly. It is explicitly not the permanent home: the automated control-plane assertions belong to the full-stack lane (M2), and the block should be removed or migrated there when that lands.
