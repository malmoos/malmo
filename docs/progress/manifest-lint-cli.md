...
Closes issue #7. App authors had no way to validate a `manifest.yml` short of opening a catalog PR and waiting on CI. This adds the author's inner-loop tool: a `malmo` CLI with a single `manifest lint` subcommand that parses + validates a manifest against the schema and sanity-checks its sibling compose file, on a dev box with no running brain. The same checks are intended to back the catalog's CI schema-lint step (`APP_STORE.md` # CI on the repo), so one binary serves both the author loop and the CI gate.

## What was done

- **`cmd/malmo`** (new) — the `malmo` CLI skeleton, package `main`, single subcommand `malmo manifest lint <path/to/manifest.yml>`. `lint(path)`:
...
- `NEXT.md` # Developer / app-author surface — `cmd/malmo` is the skeleton that item said #7 owns; the heavier `malmo install --local` surface remains deferred there.
- `CLAUDE.md` # Go discipline — `internal/` only (no `pkg/`); export-on-second-consumer, not speculatively; tests in-package; no premature abstraction (no `manifest.Lint` library function — CI invokes the binary, so the orchestration lives in `cmd/malmo`).
...
- **No CI wiring in this repo.** The binary is the reusable unit; an actual GitHub Actions step that runs `malmo manifest lint` over the catalog lands with the catalog-repo CI (`APP_STORE.md`), not here. `make build` is unchanged (it builds only the deployable host-agent + brain); the CLI builds via `go build ./cmd/malmo` and is covered by `go vet ./...` / `go test ./...`.
- **Single subcommand only.** No `malmo install --local`, no `--json` output, no globbing multiple manifests — all deferred (`NEXT.md`). The arg dispatch is a literal `manifest lint <path>` match, not a flag framework; that's enough for v1.

## Tests

`go test ./cmd/malmo/ ./internal/manifest/` green (plus the full non-PAM suite — the rename rippled cleanly, `Synthesize` still passes). `cmd/malmo/main_test.go`:
...
- **Catalog-repo CI step** that runs `malmo manifest lint` over every app directory on PR (`APP_STORE.md` # CI on the repo) — the gate this binary was built to back.
- **`malmo install --local`** and other dev subcommands (`NEXT.md` # Developer / app-author surface) — the heavier "run it on your own box" surface, still deferred.
