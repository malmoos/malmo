...
- `internal/protocol/host.go` — new `WellKnownIdentityResponse{MalmoAppUID, MalmoAppGID, MalmoSharedGID int}` with JSON tags `malmo_app_uid/malmo_app_gid/malmo_shared_gid`.
...
- `internal/hostagent/usermgr/linux.go` — `WellKnownIdentity` on `LinuxUserManager`: `os/user.Lookup("malmo-app")` for the service UID/GID, `os/user.LookupGroup("malmo-shared")` for the shared GID, each wrapped with context. These accounts are provisioned by the box build; absent on the dev box, where only the fake branch runs.
...
- Tests: `internal/hostagent/agent_test.go` (fake branch returns the fixed constants; delegate path; 500-on-error that asserts the response does not leak the `malmo-app` lookup detail); `internal/hostclient/hostclient_test.go` (round-trip over the UNIX socket). `stubUserMgr` gains a `WellKnownIdentity` stub to satisfy the updated interface.
...
- **Slice 4 — enforce in `writeOverride`/`writeEnv`.** Thread the user's per-folder elections (scope + source + subfolder) through `Install`; stamp `user:` (personal → `ResolveHome` UID/GID, household → `malmo-app` from this slice), folder bind mounts from the elected source, `group_add` with `malmo_shared_gid` for shared sources, `devices`, GPU, and `MALMO_FOLDER_*`. Add `WellKnownIdentity` to the `HostDriver` interface. Server-side validate the elections in `internal/api` (authoritative; install-plan is advisory) and audit rejections. Extend `catalog/whoami` with a `folders` declaration and verify a real dev-loop install per `feedback_verify_before_commit`.
