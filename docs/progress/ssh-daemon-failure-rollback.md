...
**Two things the review of this change corrected.** The drop-in snapshot was first taken between the key write and the render; it is now taken before either write, so a snapshot that fails costs nothing — reading it after `writeKeys` would have left a new key file live with no restore to undo it, which on a key replacement silently changes who can authenticate. And `undo` returned at the first restore error, though the two files are separate paths: it now attempts both and joins the errors, and the daemon reconcile splits by direction. **Stopping is unconditional** — `systemctl disable --now` reads neither file, and stopping is what closes `:22`, which on hosted is the only port control there is, so skipping it would leave the port open after a failed call whose previous state had nobody enabled. **Starting or reloading is gated on both files being back**, since those read what is on disk. Two further review passes got this to its current shape: the first caught that gating on the drop-in alone was not enough, because a restored drop-in points at the very path a failed key restore has left holding the new key set; the second caught that gating the stop at all was wrong, for the reason above.
...
- The half-failed undo *is* covered, in three shapes, by injecting through the runner: the failing systemctl call also removes the drop-in's directory, or the keys directory, or replaces the key file with a non-empty directory so its remove fails. Contrived, and the only way to reach those branches from a unit test.
