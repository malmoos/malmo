...
- **Not exercised on a real box.** The API tests run the real hostclient against a real `hostagent` over a real unix socket, so the host leg is the production wire rather than a mock, and one test parses `dev/control-plane/compose.yml` — the committed source `stage-control-plane.sh` copies onto a real box — so a service rename or a switch to interpolation fails in CI. (An earlier version of this test pointed at the gitignored staged copy under `dev/cloud/mkosi.extra.wiring/`, which does not exist in a fresh checkout and always skipped in CI; caught in review and fixed to read the tracked source file instead.) But no VM boot ran, so the `MALMO_CONTROL_PLANE_DIR`-set path is proven against a temp dir, not against a booted box.
