...
The trusted set is `MALMO_TRUSTED_PROXIES` (comma-separated IPs/CIDRs). Unset ⇒ loopback + Docker's bridge pool (`127.0.0.0/8`, `::1/128`, `172.16.0.0/12`): the brain container publishes no port, so every peer it can have is on the box's own `malmo-ingress` network. The default is deliberately **narrower than "the private ranges"** — see # The default set must exclude client ranges below. Set to the empty string ⇒ trust nothing, key on the peer alone. An unparseable value is **fatal at startup**, not warned-and-defaulted — silently widening the set is a security downgrade the operator never asked for, and silently narrowing it to none pools every LAN client into one shared login bucket. `cmd/brain` reads it with `os.LookupEnv` (new `envRaw`) so "set to empty" stays distinguishable from "unset", which `env()` collapses.
...
- `internal/api/clientip_test.go` — the trust boundary as a table: forged header inert without a trusted set and from an untrusted peer; right-to-left walk with forged hops behind the real one; trailing trusted hops skipped; chain split across header lines; garbage hop fails closed; all-hops-trusted falls back to the peer; IPv6 canonicalisation. Plus `ParseTrustedProxies` (bare address ⇒ /32, host bits masked, rejects junk), a check that the shipped default covers Docker/loopback peers and no routable address, and `TestDefaultTrustedProxiesKeepsLANClientsApart` — the regression test for the client-range trap below.
- `internal/api/ratelimit_test.go` — `TestRateLimit_ForgedForwardedForSharesOneBucket` drives the **real chain** (`authMiddleware` → `rateLimit`) with N distinct forged `X-Forwarded-For` values from one peer and asserts the bucket still empties. Its mirror, `TestRateLimit_TrustedProxyForwardedForIsHonoured`, asserts two LAN clients behind a trusted Caddy still get their own buckets **using the shipped default trusted set**, not one tailored to the test — a "fix" that just ignored the header, or a default that trusted the client ranges, would pool the whole LAN behind Caddy's address and let one attacker throttle everyone.
...
- `cmd/brain/main_test.go` — the env-var wiring: unset ⇒ the default set, explicitly empty ⇒ trust nothing, explicit spec ⇒ parsed. The unparseable case calls `fatal()` and exits the process, so it is deliberately uncovered.
...
### The default set must exclude client ranges

The first cut of this change defaulted the trusted set to "loopback + the private ranges", mirroring Caddy's `private_ranges` shortcut. That was wrong, and automated review on the PR caught it. A trusted hop is **skipped** during the right-to-left walk, so on an appliance the walk went: peer `172.18.0.3` (Caddy, trusted) → chain `["192.168.1.20"]` → that hop is inside `192.168.0.0/16`, therefore "a proxy", therefore skipped → nothing untrusted left → fall back to the peer. Every device in the house resolved to Caddy's address and shared **one** login bucket and **one** allowlist bucket. One device could then spend the whole household's budget — the exact failure the per-IP planes exist to prevent, reintroduced by the fix for a different one. Reproduced before changing anything (`192.168.1.20` behind `172.18.0.3` ⇒ key `172.18.0.3`).

The default is now loopback + `172.16.0.0/12` — Docker's default bridge pool, where the box's Caddy actually lives — and `10.0.0.0/8` / `192.168.0.0/16` are deliberately absent, because those are where *clients* live. The trusted set is not "addresses on the local network"; it is "addresses that are proxies", and on this box that is a much smaller thing. A box whose Docker network lands outside `172.16/12` — moved deliberately, or pushed there by pool exhaustion on a dev host with many existing networks — or a LAN that itself uses `172.16/12`, hits the milder inverse (an untrusted peer ⇒ header ignored ⇒ everyone keyed on Caddy's address, or a LAN client mistaken for a proxy) and wants `MALMO_TRUSTED_PROXIES` set explicitly. `TestDefaultTrustedProxiesKeepsLANClientsApart` pins the behaviour against the shipped default.
