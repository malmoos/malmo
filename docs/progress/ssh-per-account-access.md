...
**Deleting an SSH-disabled account raced a self-service enable (Greptile).** The delete read the account's SSH state and only took the SSH lock when it came back enabled. An account that was off therefore ran the whole delete unlocked. The user's session stays valid until the delete cascades it, so a request that was already elevated could turn SSH on in that window: the host got the account and its key file, then the cascade dropped the brain's rows, and the leftover would be handed to the next user with the same name. The lock is now taken before the read and held until the account is gone, on every delete. A write that was waiting on it finds the user row gone and fails its foreign key before it can reach the host. A test holds the lock and asserts the delete of a disabled account blocks on it.
...
- **Deleting a user revokes their SSH first.** The brain's `ssh_access` and `ssh_keys` rows cascade away with the account, but the host keeps its own copy: the account stays in sshd's `AllowUsers` and its key file stays in `/etc/ssh/malmo-authorized-keys`. A later user with the same name would inherit it. `deleteUser` now pushes a disable before removing the account, but only for accounts that were actually enabled — a disabled one was never sent to the host, and calling on every delete would fail on a box with no sshd installed. The SSH lock is held across the whole delete either way, so the account cannot turn SSH back on while it is being removed.
- **The brain serialises its SSH writes with one process-wide lock, not a revision.** Each write reads the account's state, decides, commits and pushes under `Server.sshWrites`, so two overlapping requests can no longer commit in one order and land on the host in the other. One lock for all accounts is enough here — these are rare panel actions — but it is in-process only. It would not hold if the brain ever ran as more than one process, which nothing plans today; a revision column on `ssh_access` is the durable version of the same guard.
- **A failed revocation is rolled back, not retried.** Removing a key commits in the brain and then pushes; if the push fails the row is restored, so the two sides still agree and the user can repeat the delete once the host is back. The key stays live on the host in the meantime, which no rollback can change — only a reconcile loop could, and none exists yet.
