...
- **Boot + hourly loop** (`cmd/brain/main.go`). One prune at startup, then `notificationPruneLoop` on a ticker — structurally a copy of the existing `storageHealthPollLoop`, sharing the same `pollCtx` so it shuts down with the rest. Cadence is `MOLMA_NOTIFY_PRUNE` (default `1h`); retention is housekeeping, not latency-sensitive. Every prune is best-effort: a failure logs at `warn` and the next tick retries — an unbounded table degrades gracefully (the bell just carries more history), so it never blocks boot or serving.
...
- If retention ever needs to be visible/tunable by the user, the env knob (`MOLMA_NOTIFY_PRUNE`) and the two constants (`notifyMaxAgeDays`, `notifyMaxRows`) are the seams — no UI for them in v1.
