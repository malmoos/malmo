...
- `internal/lifecycle/lifecycle_folders_test.go` — `TestInstallCustomFolders_TargetDestination`: a Door-2 grant binds the scope-derived source to the typed target (`/srv/malmo/shared/Documents:/photoprism/originals:rw`) and `MALMO_FOLDER_DOCUMENTS` reflects the target. `TestInstallHappyDoor2` updated for the new `CustomSpec.Permissions`.
