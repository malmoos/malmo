...
- **The path-matcher behaviour is asserted, not yet observed.** The bypass table expects Caddy to match a cleaned, decoded path, so every entry 302s. That is what Caddy documents, but this repo has never depended on it before, and the assertion is CI-only (the cloud lane needs root + KVM). A red boot here is a finding, not a flake — it would mean the matcher and the app can disagree about what a path is, and the fix would be a deny rule in front of the carve-out rather than a looser assertion.
