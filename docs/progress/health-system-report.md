...
Changes:
- **`internal/protocol/host.go`** — added `HealthCategory` typed enum (5 values) + `SystemHealth{checked_at, categories: map[HealthCategory][]Finding}` (the GET /v1/health/system wire shape). Added `InstanceKey` to `Finding` (service-down is per-unit). Kept `StorageHealth` as the on-disk `/run/molma/health/storage.json` boot-artifact shape (host-agent folds it into the report's storage category — the boot reporter and `molma-storage-verify` are unchanged).
