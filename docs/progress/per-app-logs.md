...
- `internal/lifecycle/lifecycle.go` — `MainContainerName(id)` resolves `malmo-<id>-<main_service>` from the on-disk instance manifest, the container the brain hands to the follow.
...
- **Replica-qualified container name** so real-box logs match (`malmo-<id>-<service>-1`), or a `CONTAINER_NAME` prefix match in `journalsource`. This is the one gap that blocks the feature on a real host (the fake works today).
