...
- **Not yet in an automated CI gate — runs in the local full lane, like `frozen`.** The mint→verify plumbing and the bash parsing are unit-tested (`mkassertion/main_test.go`; the HTTP/cookie/status greps were exercised against synthetic responses), but the full boot needs mkosi + `/dev/kvm` (`make test-cloud-qemu`). The `CI / Cloud image` workflow's boot gate runs only `unseeded seeded` (to gate publish without coupling to orthogonal boots), so the `access` boot — like `frozen` — runs only in the default full local run, not that gate. Wiring an automated CI run of the `access` boot (without coupling it to the publish path, and after characterizing its timing under CI's TCG-only QEMU) is a follow-up; #306 already shipped the `restricted` default, so this proof does not block it.
