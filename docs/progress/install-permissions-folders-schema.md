...
- Source (personal `~/<Folder>/` vs shared `/srv/molma/shared/<Folder>/`) is **not** parsed or validated — it is deliberately not a manifest concern.
...
- **No host-side source resolution.** The brain can't read `/etc/passwd`; resolving the owner's home/UID and the `molma-app` service identity + `molma-shared` GID is slice 2.
...
- **Slice 2 — host resolves owner home + UID/GID.** New `GET /v1/users/{username}/home` across `protocol`, `hostagent` (real + fake), `hostclient`, and the `HostDriver` interface; surface the `molma-app` service UID + `molma-shared` GID. Documented in `BRAIN_HOST_PROTOCOL.md`. Gates slice 4.
...
- **Slice 4 — enforce in `writeOverride`/`writeEnv`** (all services): `user:`, folder bind mounts from the elected source, `group_add` for shared sources, `devices`, GPU, `MOLMA_FOLDER_*`. Verify a real dev-loop install per `feedback_verify_before_commit`.
