...
**`wireApp` gained `ExternalCosts`**, reusing `manifest.ExternalCost` rather than re-declaring it, which is what reproduces the control plane's marshalling exactly.
...
- `go build`, `go vet`, `gofmt`, and `go test ./internal/catalog/` green.
