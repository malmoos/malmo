...
- **Post-review hardening** (PR #195 feedback, CANARY v25→v26): all logic-equivalent on the green path — `resolveOffline` now wraps the inspect error too (no longer masks a daemon-down/corrupt-store cause behind "registry unreachable"); the bundle pulls whoami **by digest** then re-tags (a moved tag can't silently bake the wrong bytes); `Content-Length` uses the byte count (`wc -c`) not the shell char count; the offline unit tests use `ScopePersonal` to mirror the VM; `envBool`'s log fields and the brain-vs-host-agent divergence are documented. A re-run to re-confirm at v26 is recommended but low-risk (the green path is unchanged).
