...
- **Public acme-dns API face — now deployed cloud-side (2026-06-23, `malmoos/cloud` #14).** The cloud control-plane VM fronts acme-dns with Caddy, exposing `/update` + `/health` over real Let's Encrypt TLS for `auth.malmo.network` (`/register` stays loopback-only, 404 on the public face) and answering the authoritative `:53` face publicly. `https://auth.malmo.network` is a **confirmed live endpoint**, so the box-side default needs no change (still overridable via `MALMO_ACMEDNS_ENDPOINT`). Real issuance against it is still pending the joint cloud #6 / CL6 run (see the "Not verified against real ACME" item above).
