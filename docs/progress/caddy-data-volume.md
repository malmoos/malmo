...
- `ENVIRONMENT.md` # Networking & discovery — as built gains the same fact where the wildcard mechanism is described, with the shared-registered-domain budget spelled out, since that is what makes it matter on hosted.
...
- **`ENVIRONMENT.md` # Networking & discovery still says "one challenge issues the combined cert"** in the wildcard bullet, which `DECISIONS.md` 2026-07-08 superseded — Caddy issues one cert per SAN and only the wildcard goes through acme-dns. Noticed while editing next to it, left alone as out of scope and filed as [#437](https://github.com/malmoos/malmo/issues/437).
...
- Fix the stale "one challenge issues the combined cert" line in `ENVIRONMENT.md` # Networking & discovery — filed as [#437](https://github.com/malmoos/malmo/issues/437).
