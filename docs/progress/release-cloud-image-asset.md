...
3. Resolves the target tag as `v$(cat VERSION)` — the same construction the tag-assert step and `release.yml`'s auto-tagging already use — and creates the GitHub Release if it doesn't exist yet (covers a `workflow_dispatch` publish run against an untagged commit; the two paths that matter in practice, a human tag push and the `workflow_call` from the release automation, always have the Release already in place).
4. Uploads with `gh release upload --clobber`, so re-running a publish (e.g. retrying a failed run) replaces the asset instead of erroring on "already exists."
