...
- **Per-issue audit records.** Change `ApplyStorageFindings` return type to carry affected IDs so audit records can target `{kind: "health_issue", id: "<id>"}` instead of emitting a bulk count record.
