...
## Public acme-dns API face for hosted boxes (cross-repo, gates #207 acceptance)

C3b (#207, [hosted-wildcard-cert.md](docs/progress/hosted-wildcard-cert.md)) configures the hosted box's Caddy to push its `_acme-challenge` TXT to an acme-dns endpoint — a box-side constant `MALMO_ACMEDNS_ENDPOINT`, default `https://auth.malmo.network`. But the cloud deploy currently binds acme-dns's HTTP API to `127.0.0.1:4443` (internal `/register` only); **no public face is deployed**, so the default is a chosen value, not a confirmed endpoint, and no box can actually renew yet. The cloud side must expose a public acme-dns API (auth-domain TLS + the `/update` path reachable from tenant boxes) and confirm the canonical URL (filed as `malmoos/cloud` #14); pinned jointly at cloud #6 / CL6. Until then the box-side wiring exists but is unexercised.
