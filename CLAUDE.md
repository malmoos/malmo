...
- `DISCOVERY.md` — LAN discovery & mDNS. Avahi as the publisher; per-app A records (`<slug>.malmo.local`) written by the install reconciler as `/etc/avahi/services/*.service` files, sibling to Caddy site blocks. LAN-only interface scoping. `.local` works on macOS/iOS/Windows-with-Bonjour/Linux; **Android browsers don't resolve `.local`**, which is the real reason the secure-URL path in `MALMO_NETWORK.md` exists. No wildcards, no CNAME tricks, no private CA for `.local`.
...
- **Avahi as the LAN publisher; per-app `.local` records owned by the reconciler.** No mDNS wildcards exist; each app slug is a real announced name, written by host-agent as a static service file alongside the Caddy site block. LAN interfaces only (not mesh, not Docker bridges). `.local` is HTTP-only by definition (no public DNS → no Let's Encrypt) and Android browsers don't resolve it — secure URLs are the compatibility path. See `DISCOVERY.md`.
