...
// an attacker: on hosted, apps live at <slug>.<box-id>.malmo.network and the
// dashboard at <box-id>.malmo.network, which are same-site under a registrable
...
func TestNoCORSHeadersOnAPIResponse(t *testing.T) {
	...
	req.Header.Set("Origin", "https://photos.box-1.malmo.network")
