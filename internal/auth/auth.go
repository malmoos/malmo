...
// Issue mints a new session for userID. Caller sets the resulting cookie on
// the response.
func (m *Manager) Issue(userID string) (store.Session, error) {
	...
	now := m.Clock()
	sess := store.Session{Token: token, UserID: userID, CreatedAt: now, LastSeenAt: now}
...
// token → ErrInvalidSession. A session whose user has been removed is also
// ErrInvalidSession (defensive — the FK cascade should have killed the
// session row already, but we don't want to leak a deleted user back to a
...
func (m *Manager) Validate(token string) (Identity, error) {
	...
	sess.LastSeenAt = now
	return Identity{User: user, Session: sess}, nil
}
